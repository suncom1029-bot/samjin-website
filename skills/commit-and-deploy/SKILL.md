...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"commit-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"commit-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
- Ship/deploy/PR → invoke /ship or /commit-and-deploy
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"commit-and-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for /commit-and-deploy is not yet implemented. Run `/ship` to create the MR, then merge manually via the GitLab web UI." Do not proceed.

# /commit-and-deploy — Merge, Deploy, Verify
...
## User-invocable
When the user types `/commit-and-deploy`, run this skill.

## Arguments
- `/commit-and-deploy` — auto-detect PR from current branch, no post-deploy URL
- `/commit-and-deploy <url>` — auto-detect PR, verify deploy at this URL
- `/commit-and-deploy #123` — specific PR number
- `/commit-and-deploy #123 <url>` — specific PR + verification URL
...
the ones listed below. The user said `/commit-and-deploy` which means DO IT — but verify
readiness first.
...
gh auth status
```
If not authenticated, **STOP**: "I need GitHub CLI access to merge your PR. Run `gh auth login` to connect, then try `/commit-and-deploy` again."
...
Check whether this project has been through a successful `/commit-and-deploy` before,
...
**If FIRST_RUN:** This is the first time `/commit-and-deploy` is running for this project. Before doing anything irreversible, show the user exactly what will happen. This is a dry run — explain, validate, and confirm.
...
**If A:** Tell the user: "Great — I've saved this configuration. Next time you run `/commit-and-deploy`, I'll skip the dry run and go straight to readiness checks. If your deploy setup changes (new platform, different workflows, updated URLs), I'll automatically re-run the dry run to make sure I still have it right."
...
**If C:** **STOP.** "Running `/setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to CLAUDE.md so I'll know exactly what to do next time. Run `/commit-and-deploy` again when that's done."
...
```
If `CONFLICTING`: **STOP.** "This PR has merge conflicts with the base branch. Resolve the conflicts and push, then run `/commit-and-deploy` again."
...
3. If drift is detected (a PR landed ahead of us and `BRANCH_VERSION < NEXT_SLOT`): **STOP** and print exactly:
   ...
   Exit non-zero. Do NOT auto-bump from `/commit-and-deploy` — rerunning `/ship` is the clean path (it already handles VERSION + package.json + CHANGELOG header + PR title atomically via Step 12 ALREADY_BUMPED detection).
...
and tell the user: "I found and fixed a few issues during the review. The fixes are committed — run `/commit-and-deploy` again to pick them up and continue where we left off."

**If no issues found:** Tell the user: "Review checklist passed — no issues found in the diff."

**If B:** **STOP.** "Good call — run `/review` for a thorough pre-landing review. When that's done, run `/commit-and-deploy` again and I'll pick up right where we left off."
...
- If reviews are stale: "Run `/review` or `/autoplan` to review the current code, then `/commit-and-deploy` again."
...
Then tell the user: "Staging looks good. When you're ready for production, run `/commit-and-deploy` again."
**STOP.** The user can re-run `/commit-and-deploy` later for production.
...
```json
{"skill":"commit-and-deploy","timestamp":"<ISO>","status":"<SUCCESS/REVERTED>","pr":<number>,"merge_sha":"<sha>","merge_path":"<auto/direct/queue>","first_run":<true/false>,"deploy_status":"<HEALTHY/DEGRADED/SKIPPED>","staging_status":"<VERIFIED/SKIPPED>","review_status":"<CURRENT/STALE/NOT_RUN/INLINE_FIX>","ci_wait_s":<N>,"queue_s":<N>,"deploy_s":<N>,"staging_s":<N>,"canary_s":<N>,"total_s":<N>}
```
...
- **Single-pass verification, not continuous monitoring.** `/commit-and-deploy` checks once. `/canary` does the extended monitoring loop.
